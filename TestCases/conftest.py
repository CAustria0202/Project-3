...
def get_config():
    ...
    config_path = os.path.join(base_dir, "..", "config.yaml")  # Adjust to go one level up to find config.yaml
    ...
    browser = os.getenv('BROWSER', config.get("settings", {}).get("browser", "chrome"))
    url = os.getenv('URL', config.get("settings", {}).get( "url", "https://google.com/"))
    head = os.getenv('HEADLESS', str(config.get("settings", {}).get("head", "False")).lower()) in ["true", "1"]
